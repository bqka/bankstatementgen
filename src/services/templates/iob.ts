...
const formatIobDate = (value: string | Date) => {
...
// Indian Overseas Bank logo - Official SVG
const buildIobLogo = () => `
  <div style="display: inline-block; margin-bottom: 10px;">
    <svg
      ...
      <g
         id="#ffffffff"
         transform="translate(-46.83,-426.89201)" />
      <g
         id="#00adeeff"
         transform="matrix(0.09498769,0,0,0.09498769,-3.5751353,-2.7521749)">
        <path
...
// Helper to render a page header
const renderPageHeader = (statement: Statement, isFirstPage: boolean) => {
  const { details, meta } = statement;
  ...
  return `
    ...
    <div style="text-align: center; margin-bottom: 15px;">
      ${buildIobLogo()}
      <div style="font-size: 12px; font-weight: bold; margin-top: 5px;">Indian Overseas Bank</div>
    </div>
    
    <div style="text-align: right; margin-bottom: 10px; font-size: 9px;">
      <div><strong>Report Generation Date & Time :</strong> ${format(new Date(meta.generatedAt), 'dd-MM-yyyy')} 17:24:32</div>
    </div>
    
    <div style="text-align: center; font-size: 11px; font-weight: bold; margin-bottom: 15px;">
      STATEMENT OF THE ACCOUNT FOR THE PERIOD OF : ${format(new Date(startDate), 'yyyy-MM-dd')} to ${format(new Date(endDate), 'yyyy-MM-dd')}
    </div>
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 9px; margin-bottom: 15px;">
      <tr>
        <td colspan="2" style="border: 1px solid #000; padding: 5px; background: #f0f0f0; font-weight: bold;">CUSTOMER DETAILS</td>
        <td rowspan="8" style="border: 1px solid #000; padding: 5px; background: #f0f0f0; font-weight: bold; width: 35%; vertical-align: top;">Branch Address :<br/><br/>${details.branchAddress || 'PLOT NO 7 and 8 PIPLIYA BAJKHAN,PIPLIYA BAJKHAN ROAD BHANPUR,HUZUR,BHOPAL'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold; width: 25%;">Customer ID</td>
        <td style="border: 1px solid #000; padding: 5px; width: 40%;">: ${details.customerId || '518437997'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Account No</td>
        <td style="border: 1px solid #000; padding: 5px;">: ${details.accountNumber || ''}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Account Holder Name</td>
        <td style="border: 1px solid #000; padding: 5px;">: ${details.accountHolderName || ''}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Contact No</td>
        <td style="border: 1px solid #000; padding: 5px;">: ${details.mobileNumber || '917869324100'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Email ID</td>
        <td style="border: 1px solid #000; padding: 5px;">:</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">IFS Code</td>
        <td style="border: 1px solid #000; padding: 5px;">: ${details.ifscCode || 'IOBA0003133'}</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Nominee</td>
        <td style="border: 1px solid #000; padding: 5px;">: NA</td>
      </tr>
      <tr>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold;">Branch Code</td>
        <td style="border: 1px solid #000; padding: 5px;">: ${details.branchCode || '3133'}</td>
        <td style="border: 1px solid #000; padding: 5px; font-weight: bold; vertical-align: top;">Address of Customer :<br/><br/>${details.address || ''}</td>
      </tr>
    </table>
    ` : ''}
  `;
...
const renderTransactionRows = (transactions: any[]) => {
  return transactions.map((txn) => {
    // Format description with line breaks for UPI transactions
    let formattedDescription = txn.description;
    
    // Add line breaks for better UPI readability
    if (formattedDescription.includes('UPI/')) {
      // Break after UPI/ref and before /To:
      formattedDescription = formattedDescription.replace(/\/To:/g, '<br/>To:');
    }
    
    return `
    <tr>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: center;">${formatIobDate(txn.date)}<br/>(${formatIobDate(txn.date)})</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; line-height: 1.4;">${formattedDescription}</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: center;">${txn.reference || ''}</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: center;">Transfer</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: right;">${txn.debit > 0 ? formatAmount(txn.debit) : ''}</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: right;">${txn.credit > 0 ? formatAmount(txn.credit) : ''}</td>
      <td style="padding: 3px 5px; border: 1px solid #000; font-size: 8px; text-align: right;">${formatAmount(txn.balance)}</td>
    </tr>
  `;
  }).join('');
};

// Helper to render a complete page
const renderIobPage = (statement: Statement, pageTransactions: any[], isFirstPage: boolean, isLastPage: boolean) => {
  const finalBalance = statement.transactions[statement.transactions.length - 1]?.balance || 0;
  
  return `
    <div class="page">
      ${renderPageHeader(statement, isFirstPage)}
      
      <!-- Transaction Table -->
      <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
        <thead>
          <tr>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 10%;">Date(Value<br/>Date)</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 35%;">Particulars</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 10%;">Ref No.<br/>/Cheque No</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 10%;">Transaction<br/>Type</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 11%;">Debit(Rs)</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 11%;">Credit(Rs)</th>
            <th style="border: 1px solid #000; padding: 5px; text-align: center; background: #f0f0f0; font-size: 9px; width: 13%;">Balance(Rs)</th>
          </tr>
        </thead>
        <tbody>
          ${renderTransactionRows(pageTransactions)}
        </tbody>
      </table>
      ...
      <div style="margin-top: 20px; font-size: 9px;">
        <strong>Effective available balance as on ${format(new Date(statement.meta.generatedAt), 'dd-MM-yyyy')} 17:24:32 is INR ${formatAmount(finalBalance)}</strong>
      </div>
      
      <!-- Footer -->
      <div style="margin-top: 30px; font-size: 8px; line-height: 1.4;">
        <div><strong>**This is a computer generated statement and does not require a signature.</strong></div>
      </div>
      ` : ''}
    </div>
  `;
...
export const renderIobTemplate = (statement: Statement) => {
  const transactions = statement.transactions || [];
  const allPages: string[] = [];
  
  // First page with customer details
  const firstPageTransactionCount = Math.min(TRANSACTIONS_PER_PAGE, transactions.length);
  const firstPageTransactions = transactions.slice(0, firstPageTransactionCount);
  
  const page1Html = renderIobPage(statement, firstPageTransactions, true, transactions.length <= TRANSACTIONS_PER_PAGE);
  allPages.push(page1Html);
  
  // If there are more transactions, create middle pages
  if (transactions.length > TRANSACTIONS_PER_PAGE) {
    const remainingTransactions = transactions.slice(TRANSACTIONS_PER_PAGE);
    const numberOfMiddlePages = Math.ceil(remainingTransactions.length / TRANSACTIONS_PER_PAGE);
    
    for (let i = 0; i < numberOfMiddlePages; i++) {
      const startIdx = i * TRANSACTIONS_PER_PAGE;
      const endIdx = Math.min(startIdx + TRANSACTIONS_PER_PAGE, remainingTransactions.length);
      const pageTransactions = remainingTransactions.slice(startIdx, endIdx);
      const isLastPage = (i === numberOfMiddlePages - 1);
      
      const pageHtml = renderIobPage(statement, pageTransactions, false, isLastPage);
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Indian Overseas Bank - Account Statement</title>
      <style>
        @page { 
          margin: 1cm 0.8cm; 
          size: A4;
