...
const formatUnionDate = (value: string | Date) => {
...
export const renderUnionTemplate = (statement: Statement) => {
  ...
  const transactionRows = transactions
    .map(
      (txn, index) => {
        const rowBg = index % 2 === 0 ? '#ffffff' : '#e8f2fc';
        
        // Format description for UPI transactions with line breaks
        let formattedDescription = txn.description;
        if (txn.description.includes('UPI/') && txn.description.includes('/To:')) {
          formattedDescription = txn.description.replace('/To:', '<br/>To:');
        }
        
        return `
      <tr style="background: ${rowBg};">
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 8px; white-space: nowrap;">${formatUnionDate(txn.date)}<br/>${new Date(txn.date).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 7px;">${txn.reference || 'UPI'}</td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 7px;">${txn.reference || ''}</td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 7px;"></td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 8px; text-align: right;">${txn.debit > 0 ? formatAmount(txn.debit) : ''}</td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 8px; text-align: right;">${txn.credit > 0 ? formatAmount(txn.credit) : ''}</td>
        <td style="padding: 3px 4px; border: 1px solid #ccc; font-size: 8px; text-align: right;">${formatAmount(txn.balance)}</td>
      </tr>
      <tr style="background: ${rowBg};">
        <td colspan="7" style="padding: 2px 4px; border: 1px solid #ccc; border-top: none; font-size: 7px; line-height: 1.4;">${formattedDescription}</td>
      </tr>
    `;
      }
    )
  ...
  <html>
    <head>
      <meta charset="utf-8" />
      <title>Union Bank of India - Statement of Account</title>
      <style>
        @page { 
    ...
    <body>
      ...
      <div class="info-section">
        <div class="left-info">
          <div style="font-weight: bold;">${details.name.toUpperCase()}</div>
          <div>MAIN KHANIYAWAD, PATTHARGATE POLICE CHOWKI</div>
          <div>MAGANAZAMPA VAKSDHARA</div>
          <div></div>
          <div style="margin-top: 10px;"><strong>City:</strong> BHAVNAGAR</div>
          <div><strong>State:</strong> GUJARAT</div>
          <div><strong>Country:</strong> INDIA</div>
          <div><strong>Zip:</strong> 390001</div>
          <div><strong>Mobile No:</strong> 9876543210</div>
          <div><strong>E-mail:</strong> riteshkhalas@gmail.com</div>
        </div>
        <div class="right-info">
          <span class="info-label">Union Bank of India</span>
          <span></span>
          
          <span class="info-label">Branch</span>
          <span>Sultanpura</span>
          ...
          <span class="info-label">CKYC Number</span>
          <span>40004646889966</span>
        </div>
      </div>

      <!-- Statement Period -->
      <div class="statement-period">
        <strong>Statement Period</strong> Date : 16/10/2025 18.01
      </div>
      <div class="statement-period">
        Records from 1 to 213. &nbsp;&nbsp;&nbsp; No more records available.
      </div>
      ...
      <table class="transaction-table">
        <thead>
          <tr>
            ...
            <th style="width: 12%;">Tran Id-I</th>
            <th style="width: 12%;">UTR Number</th>
            <th style="width: 10%;">Instr. ID</th>
            <th style="width: 12%;">Withdrawals</th>
            <th style="width: 10%;">Deposits</th>
      ...
      <!-- Footer Disclaimer -->
      <div class="footer">
        <p>For any queries, please get in touch with us on our 24 x 7 customer service help line no 1800 2222 44 4. Customers outside India need to dial +91 80 2530 2510.</p>
        <p>This is a system generated output and requires no signature. Customers are requested to immediately notify the Bank of any discrepancy in the statement</p>
        <p>TO AVAIL OUR LOAN PRODUCTS GIVE MISSED CALL AT 8010333333 OR SMS <u>LOAN</u> TO 56161</p>
      </div>
    </body>
  </html>`;
};